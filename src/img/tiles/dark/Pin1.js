import * as React from 'react';

function SvgPin1(props) {
  return (
    <svg width="1em" height="1em" viewBox="0 0 300 400" {...props}>
      <g transform="translate(0 -652.362)">
        <g transform="matrix(.89332 0 0 .89332 -207.387 341.03)">
          <circle
            r={160}
            cy={572.362}
            cx={400}
            stroke="#fff"
            strokeWidth={10.033}
            strokeLinejoin="round"
          />
          <circle
            r={145}
            cy={572.362}
            cx={400}
            fill="#fff"
            stroke="#fff"
            strokeWidth={10.033}
            strokeLinejoin="round"
          />
          <g transform="translate(.354)">
            <path
              d="M392.327 496.48c-8.272-82.935 24.083-82.938 15.288 0"
              id="Pin1_svg__a"
              fill="none"
              fillRule="evenodd"
              stroke="#000"
              strokeWidth={8.955}
              strokeLinecap="round"
              strokeLinejoin="miter"
              strokeMiterlimit={4}
              strokeDasharray="none"
              strokeOpacity={1}
            />
            <g transform="translate(-31 -64.5)">
              <use
                height="100%"
                width="100%"
                transform="rotate(20 230.025 691.537)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(40 325.532 646.348)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(60 358.702 630.653)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(80 376.35 622.303)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(100 387.87 616.852)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(120 396.42 612.807)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(140 403.39 609.509)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(160 409.52 606.608)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(180 415.28 603.883)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-160 421.04 601.158)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-140 427.17 598.258)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-120 434.14 594.96)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-100 442.69 590.914)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-80 454.21 585.464)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-60 471.859 577.113)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-40 505.028 561.419)"
                xlinkHref="#Pin1_svg__a"
              />
              <use
                height="100%"
                width="100%"
                transform="rotate(-20 600.536 516.23)"
                xlinkHref="#Pin1_svg__a"
              />
            </g>
          </g>
          <circle
            r={84.474}
            cy={572.362}
            cx={400}
            stroke="#fff"
            strokeWidth={10.24}
            strokeLinejoin="round"
          />
          <circle r={72.315} cy={572.362} cx={400} fill="#f0a023" />
          <g transform="translate(-.177 .088)" id="Pin1_svg__c">
            <path
              d="M401.332 554.245c-3.146-9.94-29.568-32.568-1.223-34.015 29.37 1.6.872 23.529-1.718 34.178"
              fill="none"
              fillRule="evenodd"
              stroke="#000"
              strokeWidth={23.11}
            />
            <path
              d="M400 555.191v-25.657"
              fill="none"
              fillRule="evenodd"
              stroke="#f0a023"
              strokeWidth={6}
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <circle
              cx={400}
              cy={527.834}
              r={4.819}
              fill="#f0a023"
              strokeWidth={3}
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M404.688 557.487l6.565-16.11"
              id="Pin1_svg__b"
              fill="none"
              fillRule="evenodd"
              stroke="#f0a023"
              strokeWidth={4}
              strokeLinecap="round"
              strokeLinejoin="miter"
              strokeMiterlimit={4}
              strokeDasharray="none"
              strokeOpacity={1}
            />
            <use
              xlinkHref="#Pin1_svg__b"
              width="100%"
              height="100%"
              transform="matrix(-1 0 0 1 799.938 0)"
              fill="none"
            />
          </g>
          <circle
            r={16.035}
            cy={572.362}
            cx={400}
            fill="#fff"
            stroke="#f0a023"
            strokeWidth={6.67}
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <circle r={8.537} cy={572.362} cx={400} fill="#f0a023" />
          <use
            height="100%"
            width="100%"
            transform="rotate(-72 399.973 572.358)"
            id="Pin1_svg__d"
            xlinkHref="#Pin1_svg__c"
            y={0}
            x={0}
          />
          <use
            height="100%"
            width="100%"
            transform="rotate(-72 399.973 572.358)"
            id="Pin1_svg__e"
            xlinkHref="#Pin1_svg__d"
            y={0}
            x={0}
          />
          <use
            height="100%"
            width="100%"
            transform="rotate(-72 399.973 572.358)"
            id="Pin1_svg__f"
            xlinkHref="#Pin1_svg__e"
            y={0}
            x={0}
          />
          <use
            height="100%"
            width="100%"
            transform="rotate(-72 399.973 572.358)"
            xlinkHref="#Pin1_svg__f"
          />
        </g>
        <circle
          cx={150}
          cy={852.362}
          r={9.522}
          fill="none"
          stroke="#000"
          strokeWidth={4}
          strokeLinejoin="round"
        />
      </g>
    </svg>
  );
}

export default SvgPin1;
